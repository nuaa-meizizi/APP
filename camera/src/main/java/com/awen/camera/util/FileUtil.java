...
public class FileUtil {
     ...
     * 系统相机图片保存路径
     */
    public static File getSavePicPath(String name,String uid) {
        File dir = FileUtil.getOrCreateAppDir(name);
        File file = null;
        if (uid!=null)
            file = new File(dir, uid);
        else
            file = new File(dir, MD5Util.hashKeyForDisk(UUID.randomUUID()+"") + ".jpg");
        try {
            if (!file.exists())
                file.createNewFile();
        } catch (IOException e) {
     ...
     * @Description: (存储文件)
     */
    public static String saveFile(byte[] fileData, String name,String uid) {
        File file = getSavePicPath(name,uid);
